...
case class InvalidNonce(current: Long, sent: Long) extends ErrorParser {
  override def toString = s"Invalid nonce parameter; on key:($current), you sent:($sent)."
}
case class InvalidPairName(name: Pair) extends ErrorParser {
  override def toString = s"Invalid pair name: $name."
}
case class InvalidMethod() extends ErrorParser {
  override def toString = "Invalid method."
}
case class NoOrders() extends ErrorParser {
  override def toString = "No orders."
}
case class NotEnoughCurrency(currency: Currency) extends ErrorParser {
  override def toString = s"Not enough ${currency.name} in the account for sale."
}
case class PriceTooHigh(first: Currency, value: BigDecimal, second: Currency) extends ErrorParser {
  override def toString = s"Price per ${first.name} must be less than $value ${second.name}."
}
case class VolumeTooLow(currency: Currency, minValue: BigDecimal) extends ErrorParser {
  override def toString = s"Volume of ${currency.name} must be higher than $minValue."
}
case class Unknown(message: String) extends ErrorParser {
